 ...
 * Copyright (C) 2021
 *
 * Huy Duong <quochuybk2010@gmail.com>
 *
...
int main(void)
{
	...
	struct usbg_config_strs c_strs = { .configuration = "ACCESSORY" };
	...
	usbg_ret = usbg_create_function(g, USBG_F_ACCESSORY, "usb0", NULL,
					&f_accessory);
	if (usbg_ret != USBG_SUCCESS) {
	...
	usbg_ret = usbg_add_config_function(c, "accessory.1", f_accessory);
	if (usbg_ret != USBG_SUCCESS) {
